...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	runningAvarageSpeed := meanSpeed(action, duration)
	return ((runningCaloriesMeanSpeedMultiplier * runningAvarageSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	//Для получения средней скорости при беге или ходьбе используйте функцию meanSpeed()
	walkingAvarageSpeed := meanSpeed(action, duration)
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow((walkingAvarageSpeed*kmhInMsec), 2)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
// duration float64 — длительность тренировки в часах.
// weight float64 — вес пользователя.
func SwimmingSpentCalories(lengthPool, countPool int, duration float64, weight float64) float64 {
	avgSwmSpd := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (avgSwmSpd + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
